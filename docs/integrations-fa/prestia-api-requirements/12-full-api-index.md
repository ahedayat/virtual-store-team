...
فهرست کامل APIهای Prestia که برای integration با Botkonak مستند شده‌اند.
...
| Support | Send Support Reply | `POST` | `/v1/messages/threads/{thread_id}/replies` | Action execution در آینده | Future | Optional | [08-support-agent-apis.md](./08-support-agent-apis.md) |
...
| Content (write) | Update Product Description | `PATCH` | `/v1/products/{product_id}` | Content action execution در آینده | Future | Optional | [06-content-agent-apis.md](./06-content-agent-apis.md) |
| Sales (write) | Apply Discount / Promotion | `POST` | `/v1/promotions` | Sales action execution در آینده | Future | Optional | [07-sales-agent-apis.md](./07-sales-agent-apis.md) |
...
| Inferred | 15 |
| Optional | 7 |
| Open question | 0؛ برای فهرست سؤال‌های باز به [11-api-priority-and-mvp-scope.md](./11-api-priority-and-mvp-scope.md) مراجعه کنید |
...
این index، APIهایی را تجمیع می‌کند که از موارد زیر استخراج شده‌اند:

- `backend/catalog/internal_views.py` و `backend/accounts/internal_urls.py`
- `backend/catalog/services.py` و `backend/catalog/context.py`
- `agents/sales/django_fetch.py` و `agents/support/django_fetch.py`
- `agents/coordinator/nodes.py`
- `docs/agents/*.md` و `docs/phases/step-3.*.md`
